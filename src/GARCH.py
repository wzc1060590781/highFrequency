...
def get_ARMA_residual():
    ...
    spread_df["ARMA_resid"] = model.resid
    spread_df["ARMA_resid_square"] = np.square(model.resid)
...
def garch(judge_by = "bic"):
    ...
    results = []
    for param in parameters_list:
        am = arch_model(spread_df["ARMA_resid_square"],vol="GARCH",p = param[0],o=0,q=param[1])  # 默认模型为GARCH（1，1）
